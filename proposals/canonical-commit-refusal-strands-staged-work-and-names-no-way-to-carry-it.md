...
received: 2026-08-19
release: v1.73.2
reason: "accepted as reported, including the shape the reporter held loosely. Reproduced at v1.73.1: with a fully-staged tree `git diff > patch` wrote a 0-byte file and exited 0, so the save looks successful and the discard that follows takes the only copy. All four of the reporter's fixture claims were re-verified independently rather than taken on trust - the empty patch, `stash push --include-untracked` removing a peer's untracked file from their tree, `stash push -- <paths>` leaving it alone, and a cross-worktree `stash pop --index` restoring both content and the staged split. The precedent they raised against themselves (update-writes-canonical-against-the-worktree-convention, where wording-only was declined) was checked and does NOT apply here: it was declined because the wording named a route the engine's own gate then refused, and every step of this carry is a stash and a pop, so nothing in it meets a gate. Their reason for not escalating to a structural fix is accepted and adopted as the reason for not building one - a gate that silently relocates your files is worse to be surprised by than one that only tells you how. Shipped slightly stronger than the sketch in one respect the report pointed at itself: the guard prints the operator's ACTUAL staged paths rather than a placeholder, because it is the one component that knows the staged set at that instant, which makes the line runnable instead of a template to fill in. Two things the report did not ask for: the advice is conditional on work actually being staged (via `git commit <path>` the guard is reached with an empty index, where `git diff` WOULD capture the work and the warning would be false), and the path list is `--no-renames -z` plus %q-quoted so a rename yields both names and a path holding a space survives into a pasted command. The sweep the report invited found no siblings: the other sites that send an operator to a worktree (update.sh's deferred-page and catalog blocks) avoid creating this state rather than sharing it - they decline to write and print the values to apply - and the pre-commit lint refusal implies no relocation. The gate asserts the property by RUNNING the printed line rather than matching wording, was proven red at HEAD, and an unscoped `git stash push` near-miss was built and rejected."
---
