---
slug: bump-pinned-tokenizers-0-23-2
outcome: accepted
reason: "accepted as proposed and shipped as proposed — one pin, one commit, nothing else moved. The block arrived with the whole 3.12–3.14 range covered by a nine-venv measurement taken before either pin advanced (baseline, tokenizers-only, sibling-only, on each interpreter): pip check clean, 768-dim non-degenerate vectors, and the 0.23.2 vectors bit-identical to the 0.23.1 baseline on every interpreter. Intake confirmed the file-level claims against PyPI metadata (wheel form, requires_python, the fastembed range) and the diff, and rag_deps_check.load_pins() still parses the file. The sibling pin lands in its own commit in the same release, as the v1.73.1 precedent did."
received: 2026-09-03
---
...
Title: Bump the pinned RAG embedder dep tokenizers 0.23.1 -> 0.23.2

Problem:

`scaffold/rag-requirements.txt` pins `tokenizers==0.23.1`. `0.23.2` was released to PyPI on
...
Compatibility checked before proposing, so intake need not re-derive it:
  ...
  - `0.23.2` ships compiled `cp310-abi3` wheels (stable ABI, so one wheel covers 3.10 through 3.14) for macOS arm64/x86_64, manylinux and musllinux on every common arch, and Windows — no interpreter in the documented range builds from source.
...
exists), then the set with only `tokenizers` at `0.23.2`:
  ...
  - The `0.23.2` vectors are bit-identical to the `0.23.1` baseline on every
    interpreter (cosine 1.0000000, max component delta 0.0, on all three texts).
    Unlike the `onnxruntime` pin, whose kernels perturbed embeddings without changing their dimension at 1.28.0 -> 1.29.0, this tokenizer patch leaves the vectors unchanged to the bit — checked rather than assumed, because a tokenizer change CAN alter the token ids an embedding is computed from. An existing vector store stays valid and no reindex is implied.
...
Bump the single pin in `scaffold/rag-requirements.txt` to `tokenizers==0.23.2`,
...
Acceptance criteria:
  ...
  - `scaffold/rag-requirements.txt` pins `tokenizers==0.23.2`, with no other pin
