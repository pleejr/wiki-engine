...
- **`vault-worktree.sh integrate` no longer rebases onto a stale local `<main>`.** It fetched `origin/<main>` and then rebased onto the **local** ref, so the fetch was never consulted. A vault publishing by branch → pull request → **squash-merge** gets a new object on origin that its local `<main>` is not an ancestor of — so *every* integrate based work on a superseded history and printed `integrated <branch> -> <main>`, leaving canonical divergent and the **next** publication unmergeable, far from the tool that caused it. `integrate` now reconciles canonical with the ref it just fetched before choosing a base: fast-forward when behind; **adopt** origin when the divergence is content-equal (the squash signature) *and* canonical is clean; otherwise refuse with exit 3 rather than silently pick a side. Reported from a consumer vault. `Proposal: vault-worktree-integrate-rebases-onto-stale-local-main`
  ...
  - The `reset --hard` that adopts origin is gated on a **clean canonical**, because it would otherwise discard uncommitted work without complaint. CI asserts the adopt path, the real-divergence refusal, and that a dirty canonical refuses with its edit intact.
