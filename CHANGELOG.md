...
**What gets a tag:** the engine is consumed by *pinning a tag* (a vault's `engine/` submodule; `update.sh` advances tag→tag), so tag + release **only** when a change touches what a pinned consumer runs — `skills/`, `bin/`, `SCHEMA.md`, `scaffold/`, the `CLAUDE.md` router (`LICENSE`/legal too). **Docs-only** changes (`README`, `USAGE`, comments, this file's prose) land on `main` **untagged** — consumers read those from `HEAD`/their clone, never through the pin — and ride along under `## [1.44.1] — 2026-07-28

Patch — three defects in the v1.44.0 proposal queue, all found by **using it**: the first real consumer submission could not publish, the second carried the first's file, and merging one turned `main` red. Adopt with `bin/adopt.sh` or `update.sh`; no migration. Reported through the queue itself as `push-verb-omits-git-push` and `submit-branches-from-head-not-main`.
...
## [Unreleased]` into the next functional release.
