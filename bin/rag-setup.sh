...
# nothing external after the one-time install. Needs Python 3.12–3.14 + pip + one model
...
VENV="$WIKI/.rag/venv"
MIN_PY=312 MAX_PY=314   # supported Python range for the pinned bge stack (X*100+Y)
...
# Echoes the chosen interpreter, or "" if none in 3.12–MAX is found.
choose_python() {
  local v c p
...
pinned_install_hint() {
  cat >&2 <<EOF
rag-setup: could not provision the pinned bge stack (needs Python 3.12–3.14; this
  machine's python3 is $(python3 -V 2>&1 | awk '{print $2}') and no in-range interpreter was found).
  Remedies:
    • Install/select a Python in 3.12–3.14 (pyenv or Homebrew), then rerun.
...
if [ ! -x "$VENV/bin/python" ]; then
  ...
  # The pinned bge path needs 3.12–3.14, so pick an in-range interpreter or guide out.
  if [ -n "${RAG_PIP_PKG:-}" ]; then
    PY=python3
  else
    PY="$(choose_python)"
    [ -n "$PY" ] || { pinned_install_hint; exit 1; }
    [ "$PY" != "python3" ] && echo "rag-setup: default python3 is out of range 3.12–3.14; using $PY ($("$PY" -V 2>&1 | awk '{print $2}'))"
  fi
