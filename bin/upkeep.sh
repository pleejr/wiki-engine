...
# --- maintenance must leave no ACCESS FOOTPRINT ------------------------------------------
#
# `rag-capture.sh` selects a repo for the session buffer when it sees a fetch or a
# checkout inside its window — the mtime of `FETCH_HEAD` or of the `HEAD` reflog — because
# a session spent READING a repo writes nothing else. That signal means *the operator was
# here*, and this verb performs the identical filesystem action on every clone it manages,
# for maintenance. One `sync-clones` run therefore stamped the whole workspace as worked,
# and the next session-end capture recorded repos nobody had opened — asserting, in a
# buffer that feeds distillation and semantic recall, that work happened where it did not.
#
# It is the same trap v1.54.0 rejected the index mtime for: a tool perturbing the signal
# another tool reads as evidence. There it was the capture perturbing its own; here it is a
# sibling verb perturbing it from across the engine.
#
# THE FIX BELONGS HERE, not in the reader. The reader cannot tell a fetch by a person from
# a fetch by a tool — nothing in the file distinguishes them — but the tool knows exactly
# what it did, so it restores the timestamps it disturbed. Content is untouched: a reflog
# entry this verb causes stays in the reflog, honestly; only the *timing* footprint, which
# is what "the operator was here" is inferred from, is put back.
#
# What this does NOT cover, deliberately, and the docs say so: a consumer's own `fetch
# --all` sweep. Its footprint is indistinguishable from access by design, and the engine
# can only be answerable for the verbs it ships.
sc_save_mtimes() {   # sc_save_mtimes CLONE STASHDIR — copy the signal files, mtimes intact
  local gd; gd="$(git -C "$1" rev-parse --absolute-git-dir 2>/dev/null || true)"
  [ -n "$gd" ] || return 0
  mkdir -p "$2" || return 0
  local f
  # HEAD is saved as a TIMESTAMP REFERENCE, never restored: a clone with no reflog at all
  # (a fresh mirror, a fixture) gets one the moment we fast-forward it, and there is no
  # earlier mtime to put back. HEAD's own mtime is the repo's last real head change, which
  # is the honest answer to "when was this repo last actually touched" and is old.
  for f in FETCH_HEAD logs/HEAD HEAD; do
    if [ -e "$gd/$f" ]; then
      mkdir -p "$2/$(dirname "$f")"
      cp -p "$gd/$f" "$2/$f" 2>/dev/null || true
    else
      : > "$2/$(basename "$f").absent"   # remember it did not exist, so we can remove it
    fi
  done
}
sc_restore_mtimes() {  # sc_restore_mtimes CLONE STASHDIR
  local gd; gd="$(git -C "$1" rev-parse --absolute-git-dir 2>/dev/null || true)"
  [ -n "$gd" ] || return 0
  local f
  for f in FETCH_HEAD logs/HEAD; do
    if [ -e "$2/$f" ]; then
      [ -e "$gd/$f" ] && touch -r "$2/$f" "$gd/$f" 2>/dev/null || true
    elif [ -e "$2/$(basename "$f").absent" ] && [ "$f" = "FETCH_HEAD" ]; then
      # FETCH_HEAD is regenerated by every fetch and read by nobody between them; a clone
      # that had none before this verb ran should have none after. The reflog is NEVER
      # deleted — it is history a consumer may rely on, and only its mtime is ours to put
      # back.
      rm -f "$gd/$f" 2>/dev/null || true
    elif [ "$f" = "logs/HEAD" ] && [ -e "$gd/$f" ] && [ -e "$2/HEAD" ]; then
      # a reflog that did not exist until our fast-forward wrote it: date it to the repo's
      # last real head change rather than to now, so maintenance still reads as maintenance
      touch -r "$2/HEAD" "$gd/$f" 2>/dev/null || true
    fi
  done
...
case "$CMD" in
  sync-clones)
    ...
    for f in "$WIKI/repos"/*.md; do
      ...
      sc_stash="$(mktemp -d 2>/dev/null || echo "")"
      [ -n "$sc_stash" ] && sc_save_mtimes "$clone" "$sc_stash"
      if ! git -C "$clone" fetch --quiet 2>/dev/null; then
        [ -n "$sc_stash" ] && sc_restore_mtimes "$clone" "$sc_stash"
      ...
      # Every path out of the fetch restores, including the one that fast-forwarded: this
      # verb advancing a clone is still not the operator having been in it, and the reflog
      # entry the ff wrote keeps its content — only its mtime goes back.
      [ -n "$sc_stash" ] && sc_restore_mtimes "$clone" "$sc_stash"
    done
    if [ -n "$sc_check" ]; then
