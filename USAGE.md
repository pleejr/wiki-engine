...
- **`engine-proposal`** — genericize + boundary-scrub a *consumer* vault's engine-improvement idea into a self-contained, scan-verified kickoff block for the engine-dev vault; and, at the engine-dev end, **intake**: design-review an arriving proposal before choosing a shape, then file the project with the accepted/rejected findings. Forward-only: originates a new idea (never a consumer node), so unlike `crossover` there is no integrity handshake and nothing is deleted — the only shared surface is the boundary gate.
