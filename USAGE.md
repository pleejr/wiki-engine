...
| `vault-worktree.sh` | Concurrency machinery for multi-session/agent writing: `ensure` (per-session worktree), `guard` (refuse a commit in canonical — wire from `pre-commit`), `lease`/`peers` (advisory path declarations + live sessions), `claim` (advisory per-project presence, recorded on the same lease), `integrate` (locked rebase + fast-forward to `main`), `gc`/`list`. **`gc <path>`** retires one worktree now; **bare `gc`** sweeps stale worktrees, dead leases, **and orphan `wt/*` branches whose worktree is already gone** — those were previously evaluated only as a side effect of retiring a worktree, so nothing could ever reap them. A branch checked out anywhere is excluded by decision, and containment is judged by content, so every uncertain case keeps. **`ensure` never hands back a base silently behind the branch it claims to derive from:** reattaching to a surviving `wt/<session>` branch reports the distance from `origin/main` and how to rebase, and a branch whose content already landed (squash-merged included — judged by the same content test `gc` uses) is cut fresh off `origin/main` rather than reattached behind it. See the Concurrency section below. |
