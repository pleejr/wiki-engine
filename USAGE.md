...
| `lint-proposals.sh` | CI gate over `PROPOSALS.md` and the `Proposal:` commit trailers: rows well-formed and unique, aliases resolvable and unchained, declines carrying a reason, every trailer having a row — and, the load-bearing one, a `Proposal:` line written where git's trailer parser cannot see it (anywhere but the final paragraph) failing HARD rather than reading as "no trailer". Needs full history; treats a shallow clone as a failure rather than passing vacuously. |
