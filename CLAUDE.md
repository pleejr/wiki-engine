...
- **Deterministic hooks need no guard** — git, file writes, `curl` (e.g. `rag-capture.sh`) can't recurse into `claude`, so wire them freely. Ingest / refresh / checkpoint / distill still default to **in-session, on-demand**; automate them headlessly only with the guards above. The rule above is the engine's own copy and stands alone; a vault may keep a fuller write-up of the incident, but nothing here depends on one existing.
