...
- `WORK="$($WIKI_PATH/engine/bin/vault-worktree.sh ensure)"` — creates (or reuses) a per-session `git worktree` on its own `wt/<session>` branch off `origin/main` and prints its path. **Read its stderr when it reattaches:** a `wt/<session>` branch can outlive its worktree, and one that still holds work is reattached rather than reset — so `ensure` reports how many commits behind `origin/main` you are and tells you to rebase. Editing on a stale base is not visible in the returned path; it surfaces as a merge conflict on append-only pages (`log.md`, generated index regions) much later. A branch whose content already landed is cut fresh off `origin/main` instead; cheap (~0.4s, <1 MB, since only tracked text is checked out). Idempotent within a session — it keys the worktree on `$CLAUDE_CODE_SESSION_ID`, so calling `ensure` again returns the *same* worktree instead of spawning a duplicate. Opt out with `WIKI_WORKTREE=0` (writes then go straight to `$WIKI_PATH`, the legacy behavior).
