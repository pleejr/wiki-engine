...
- **`session-boot.sh`** — the engine's **single** SessionStart entrypoint. Wire this one hook and the engine owns the rest: it runs `apply-adopt.sh` (auto-wire pending features), then `session-preflight.sh` (version staleness). Deterministic; never runs `claude`; always exits 0. `adopt.sh`/`update.sh`/`new-wiki`'s adoption self-wire it (`adopt.d/10-session-boot-hook.sh`).
- **`session-banner.sh`** — shows the version verdict to the **user** in-session at start, via the hook `systemMessage` JSON field (exit 0, no "hook error" heading — the clean channel, unlike stderr+exit2; and unlike plain stdout which SessionStart routes to the model only). Instant/network-free (reads the `session-preflight.sh` cache). Complements `statusline.sh` (persistent) with a one-shot announcement and works where the statusLine is suppressed. Self-wired by `adopt.d/40-session-banner-hook.sh`.
